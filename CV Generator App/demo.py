...
class PDFCV(FPDF):
    ...
    def generate_cv(self, name, email,
        ...
        self.cell(0, 10, "Contact Inforamtion",
        ...
        self.set_font("Arial", "", 10)
        for skill in skills:
            self.cell(0, 5, f"- {skill}", new_x="LMARGIN",
                      new_y="NEXT", align="L")
...
label_education = Label(
    window, text="Education(One per line in format 'Degree': 'University')")
label_education.pack()
entry_education = Text(window, height=5)
entry_education.pack()

label_experience = Label(
    window, text="Experience(Enter one per line in format 'Job Title': 'Description')")
